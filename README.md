...
완성된 PDF를 **`public/files/portfolio.pdf`**로 올리고 `main`에 반영하세요. 다음 배포에서 포트폴리오 페이지에 **PDF 열기 / 다운로드**가 자동으로 생깁니다. 파일이 없으면 ‘준비 중’만 표시합니다. 같은 이름으로 교체하면 링크 주소가 유지됩니다.
...
포트폴리오 소개와 짧은 자기소개는 `scripts/build.mjs`에서 수정합니다. 처음 편집한 글 세 편은 기존 Notion 프로젝트 기록에서 선별·재구성했으며, 표시 날짜는 블로그 편집일입니다. Notion을 바꿔도 자동 동기화되지는 않습니다.
